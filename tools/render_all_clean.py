...
async def render_all_clean():
    ...
    print("=== START CLEAN RENDERING ALL 23 SEGMENTS ===")
    ...
    for idx, seg in enumerate(segments, 1):
        ...
        mp3_file = os.path.join(STORAGE_DIR, f"clean_tts_{idx}.mp3")
        trim_file = os.path.join(STORAGE_DIR, f"clean_trim_{idx}.wav")
        
        # Remove if exists
        for p in [mp3_file, trim_file]:
        ...
        main_p = os.path.join(STORAGE_DIR, f"segment-seg-{idx}-voice.wav")
        with wave.open(main_p, 'rb') as wf:
        ...
        print(f"Seg {idx:02d} | Slot: {orig_dur:5.2f}s | Audio: {final_dur:5.2f}s (Speed {speed_factor:.2f}x) | OK")
        
    cur.execute("UPDATE dubbing_projects SET data = ? WHERE id = 'dub-1786897570201'", (json.dumps(data, ensure_ascii=False),))
    conn.commit()
    print("ALL 23 SEGMENTS CLEANLY RENDERED & DB UPDATED!")

asyncio.run(render_all_clean())
