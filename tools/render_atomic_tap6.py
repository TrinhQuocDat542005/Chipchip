...
async def render_atomic():
    ...
    print("=== START ATOMIC RENDERING FOR TAP 6 ===")
    ...
    for idx, seg in enumerate(segments, 1):
        ...
        mp3_file = os.path.join(STORAGE_DIR, f"atomic_mp3_{idx}.mp3")
        trim_file = os.path.join(STORAGE_DIR, f"atomic_trim_{idx}.wav")
        final_gen = os.path.join(STORAGE_DIR, f"atomic_final_{idx}.wav")
        
        # 1. Edge TTS with retry
        for attempt in range(1, 6):
            try:
                comm = edge_tts.Communicate(text, "vi-VN-HoaiMyNeural")
                await comm.save(mp3_file)
                if os.path.exists(mp3_file) and os.path.getsize(mp3_file) > 500:
                    break
        ...
        subprocess.run([
            ...
            "-ar", "24000", "-ac", "1", trim_file
        ], capture_output=True, check=True)
        
        with wave.open(trim_file, 'rb') as wf:
        ...
        # 3. Apply speed
        subprocess.run([
            "ffmpeg", "-y", "-i", trim_file,
            "-filter:a", f"atempo={speed_factor:.4f}",
            "-ar", "24000", "-ac", "1", final_gen
        ], capture_output=True, check=True)
        
        with wave.open(final_gen, 'rb') as wf:
        ...
        for fname in [f"segment-seg-{idx}-voice.wav", f"segment-{idx:03d}-voice.wav", f"segment-{idx}-voice.wav"]:
            out_p = os.path.join(STORAGE_DIR, fname)
            shutil.copy2(final_gen, out_p)
            
        seg["voice_duration"] = round(final_dur, 3)
        seg["voice_speed"] = round(speed_factor, 2)
        seg["voice_url"] = f"segment-seg-{idx}-voice.wav"
        
        for p in [mp3_file, trim_file, final_gen]:
            try:
                if os.path.exists(p): os.remove(p)
            except Exception: pass
            
        status = "✅ PASS" if final_dur <= orig_dur + 0.05 else "❌ OVER"
        print(f"Seg {idx:02d} | Slot: {orig_dur:5.2f}s | Audio: {final_dur:5.2f}s (Speed {speed_factor:.2f}x) | {status}")
        
    cur.execute("UPDATE dubbing_projects SET data = ? WHERE id = 'dub-1786897570201'", (json.dumps(data, ensure_ascii=False),))
    conn.commit()
    print("ALL 23 SEGMENTS ATOMICALLY SAVED & COMMITTED!")

asyncio.run(render_atomic())
