...
async def generate_tts_with_retry(text, voice, out_path, max_retries=5):
...
async def main():
    ...
    print("================================================================================")
    print("=== THỰC THI CONCISION, RETRY TTS & SPEED FIT (<=1.18X) CHO TẬP 6 ===")
    print("================================================================================\n")
    ...
    for idx, seg in enumerate(segments, 1):
        ...
        out_wav_seg = os.path.join(STORAGE_DIR, f"segment-seg-{idx}-voice.wav")
        out_wav_000 = os.path.join(STORAGE_DIR, f"segment-{idx:03d}-voice.wav")
        
        await generate_tts_with_retry(new_text, "vi-VN-HoaiMyNeural", temp_mp3)
        ...
        subprocess.run([
            ...
            "-ar", "24000", "-ac", "1", out_wav_seg
        ], capture_output=True, check=True)
        
        shutil.copy2(out_wav_seg, out_wav_000)
        
        with wave.open(out_wav_seg, 'rb') as wf:
        ...
        is_overflow = final_dur > (orig_dur + 0.05)
        status = "🟢 SẴN SÀNG (VỪA VẶN)" if not is_overflow else f"🔴 TRÀN ({final_dur-orig_dur:.2f}s)"
    ...
    conn.commit()
    print("\n🎉 Đã cập nhật xong SQLite database và toàn bộ 23 file audio!")
